...
func main() {
	...
	// Setup upload directory with fallback
	...
	userUseCase := usecases.NewUserUseCase(userRepo, passwordSvc, jwtSvc, validationSvc, emailSvc, tokenUseCase)
	...
	// Setup router with all required controllers and use cases
	router := routers.SetupRouter(userController, tokenController, blogController, adminUseCase, jwtSvc)
